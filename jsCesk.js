  ...
  const base = cc.base === undefined ?
      {
        ...
        FunctionCreate: function (kind, ParameterList, Body, Scope, Strict, store, prototype)
        {
          var allocateResult = allocateClosure(Body, Scope, store);
          var closure = allocateResult.ref;
          store = allocateResult.store;
          return {closure, store};
        },
        MakeConstructor: function (F, store)
        {
          return store;
        }
      }
...
//      return kont.unch(new EvalState(nodes[0], benv, store));
    ...
    function evalFunctionExpression(node, benv, store, lkont, kont)
    {
      ...
      let closure;
      ({closure, store} = base.FunctionCreate("Normal", node.params, node.body, benv, strict, store));
      store = base.MakeConstructor(closure, store);
