...
function CeskBase(baseCesk)
{
  this.cesk = baseCesk
}

CeskBase.prototype.ToObject =
    function (value, store, lkont, kont, as)
    {
      return {value, store};
    }
...
function jsCesk(cc)
{
  ...
  function allocateClosure(node, benv, store, realm)
  {
    assert(realm);
    var closure = createClosure(node, benv, realm);
    var closurea = a.closure(node, benv, store);
    
    var prototype = createObject(realm.Intrinsics.get("%ObjectPrototype%"));
    var prototypea = a.closureProtoObject(node, benv, store);
    var closureRef = l.abstRef(closurea);
    prototype = prototype.add(P_CONSTRUCTOR, valueProperty(closureRef));
    store = storeAlloc(store, prototypea, prototype);
    
    closure = closure.add(P_PROTOTYPE, valueProperty(l.abstRef(prototypea)));
  ...
  {
    assert(realm);
    var obj = createObject(realm.Intrinsics.get("%FunctionPrototype%"));
    obj.Call = SetValue.from1(Call);
    return obj;
  }
  
  function createObject(proto)
  {
    let obj = new Obj(proto);
    return obj;
  }
  ...
  ObjClosureCall.prototype.applyFunction =
      function (application, operandValues, thisa, TODO_REMOVE, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
      {
        assert(realm);
        const userContext = kalloc(this, operandValues, store);
        var previousStack = Stackget(Stack(lkont, kont), stacks);
        var stackAs = stackAddresses(lkont, kont).join(this.addresses());
        var ctx = createContext(application, thisa, userContext, stackAs, previousStack, contexts, sstorei);
        return performApply(operandValues, this.node, this.scope, store, lkont, kont, ctx, effects, realm)
      }
  
  ObjClosureCall.prototype.applyConstructor =
      function (application, operandValues, protoRef, TODO_REMOVE, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
      {
        var previousStack = Stackget(Stack(lkont, kont), stacks);
  ...
  function applyProc(application, operatorValue, operandValues, thisa, benv, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
  {
    assert(stacks);
    assert(realm);
    ...
    if (errors)
    {
      ...
      {
        let throwValue = createError(l.abst1(application.callee + " is not a function"), realm);
    ...
    return operatorAs.flatMap(
        function (operatora)
        {
          ...
          const Call = operatorObj.Call;
          const callables = [...Call];
          return callables.flatMap(
              function (callable)
              {
                //if (!callable.applyFunction) {print(application, callable, Object.keys(callable))};
                return callable.applyFunction(application, operandValues, thisa, operatorObj, store, lkont, kont, effects.slice(0), stacks, contexts, sstorei, realm);
              })
        })
  }
  
  // cloned from 'applyProc', invokes 'applyConstructor' iso. 'applyFunction' on callables
  function applyCons(application, operatorValue, operandValues, benv, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
  {
    assert(realm);
    ...
    return operatorAs.flatMap(
        function (operatora)
        {
          ...
          const Call = benv.Call;
          var callables = [...Call];
          return callables.flatMap(
              function (callable)
              {
                return callable.applyConstructor(application, operandValues, protoRef, benv, store, lkont, kont, effects.slice(0), stacks, contexts, sstorei, realm);
              })
        })
  }
  ...
  {
    let prop = new Property(value, true);
    return prop;
  }
  ...
  function setProperty(objectRef, name, value, store)
  {
    let as = objectRef.addresses().values();
    let must = BOT;
    for (const a of as)
    {
      let obj = storeLookup(store, a);
      let prop = valueProperty(value);
      obj = obj.add(name, prop);
      const isArray = obj.lookup(l.abst1("isArray"));
      if (isArray !== BOT) // TODO mmm
      {
        var n = name.ToNumber();
        var i = name.ToUint32();
        if (n.equals(i))
        {
          ...
          {
            obj = obj.add(P_LENGTH, valueProperty(l.add(i, L_1)));
          }
        }
      }
  ...
  function lookupProperty(objectRef, name, store)
  {
    const effects = [];
    let as = objectRef.addresses().values();
    let value = BOT;
    while (as.length > 0)
    {
      ...
      if (proto.equals(L_NULL))
      {
        value = value.join(L_UNDEFINED);
      }
      as = as.concat(proto.addresses().values());
    }
    return value;
  }
  ...
  {
    var obj = new Obj(realm.Intrinsics.get("%ArrayPrototype%"));
    // TODO temp
    obj = obj.add(l.abst1("isArray"), L_TRUE);
    return obj;
  }
  ...
  function createContext(application, thisa, userContext, stackAs, previousStack, contexts, sstorei)
  {
    assert(contexts);
  ...
  const base = cc.base === undefined ?
      {
        ToObject: function (value, store, lkont, kont, as)
        {
          return {value, store};
        },
      }
  ...
  Property.prototype.equals =
      function (x)
      {
        ...
        return (this.value === x.value || this.value.equals(x.value))
            && this.must === x.must
      }
  ...
  Property.prototype.hashCode =
      function ()
      {
        ...
        result = prime * result + HashCode.hashCode(this.must);
        return result;
      }
  ...
  Property.prototype.toString =
      function ()
      {
        return "[" + (this.must ? "must " : "maybe ") + this.value + "]";
      }
  ...
  function FunctionCreate(kind, funNode, Scope, Strict, store, realm, prototype)
  {
    assert(realm);
    assert(realm.Intrinsics);
    var allocateResult = allocateClosure(funNode, Scope, store, realm);
    var closure = allocateResult.ref;
    store = allocateResult.store;
    return {closure, store};
  }
  
  function MakeConstructor(F, store)
  {
    return store;
  }
  
  function performApply(operandValues, funNode, scope, store, lkont, kont, ctx, effects, realm)
  {
    assert(realm);
    assert(realm.Intrinsics);
    ...
    if (names.length > 0)
    {
      ...
      nodeAddr.forEach(
          function (na)
          {
            ...
            {
              var allocateResult = allocateClosure(node, extendedBenv, store, realm);
  ...
  EvalState.prototype.next =
      function (realm)
      {
        ...
        return evalNode(this.node, this.benv, store, this.lkont, this.kont, [], realm);
      }
  ...
  KontState.prototype.next =
      function (realm, stacks, contexts, sstorei)
      {
        ...
        return lkont[0].apply(value, store, lkont.slice(1), kont, realm, stacks, contexts, sstorei);
      }
      ...
      function (realm, stacks, contexts, sstorei)
      {
        ...
        stacks.forEach(
            function (stack)
            {
              ...
              result = result.concat(frame.applyThrow(value, store, lkont2, kont, realm, stacks, contexts, sstorei));
            });
        return result;
      }
  ...
  VariableDeclaratorKont.prototype.apply =
      function (value, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        var effects = [];
        var id = this.node.id;
        var benv = this.benv;
        store = doScopeSet(id, value, benv, store, effects, realm);
...
//      VALUE = sanitize(VALUE); //TODO?
  ...
  function applyBinaryOperator(operator, leftValue, rightValue, benv, store, lkont, kont)
  {
    switch (operator)
    {
      ...
      case "instanceof":
      {
        return InstanceofOperator(leftValue, rightValue, benv, store, lkont, kont); // TODO return {VALUE, store} here and everywhere else
      }
      default:
  ...
  AssignIdentifierKont.prototype.apply =
      function (value, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        ...
        switch (node.operator)
        {
          ...
          case "+=":
          {
            var existingValue = doScopeLookup(node.left, benv, store, effects, realm);
        ...
        // }
        store = doScopeSet(node.left, newValue, benv, store, effects, realm);
  ...
  OperatorKont.prototype.apply =
      function (operatorValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        const globala = realm.GlobalObject;
        var node = this.node;
        var benv = this.benv;
        ...
        if (operands.length === 0)
        {
          ...
          {
            return applyCons(node, operatorValue, [], benv, store, lkont, kont, [], stacks, contexts, sstorei, realm);
          }
          return applyProc(node, operatorValue, [], globala, null, store, lkont, kont, [], stacks, contexts, sstorei, realm);
        }
  ...
  OperandsKont.prototype.apply =
      function (operandValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        var node = this.node;
        var benv = this.benv;
        var i = this.i;
        ...
        if (i === operands.length)
        {
          if (Ast.isNewExpression(node))
          {
            return applyCons(node, operatorValue, operandValues.addLast(operandValue), benv, store, lkont, kont, [], stacks, contexts, sstorei, realm);
          }
          return applyProc(node, operatorValue, operandValues.addLast(operandValue), thisa, null, store, lkont, kont, [], stacks, contexts, sstorei, realm);
        }
  ...
  TryKont.prototype.applyThrow =
      function (throwValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        var node = this.node;
        ...
        return evalStatementList(body, extendedBenv, store, lkont, kont, [], realm);
      }
  ...
  ObjectKont.prototype.apply =
      function (initValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        var node = this.node;
        ...
        {
          var effects = [];
          var obj = createObject(realm.Intrinsics.get("%ObjectPrototype%"));
          var objectAddress = a.object(node, benv, store, lkont, kont);
          for (var j = 0; j < i; j++)
          {
            var propertyName = l.abst1(properties[j].key.name);
            obj = obj.add(propertyName, valueProperty(initValues[j]));
          }
...
//        effects.push(allocObjectEffect(objectAddress));
  ...
  ArrayKont.prototype.apply =
      function (initValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        ...
        {
          var arr = createArray(realm);
          var arrAddress = a.array(node, benv, store, lkont, kont);
          for (var j = 0; j < i; j++)
          {
            var indexName = l.abst1(String(j));
            arr = arr.add(indexName, valueProperty(initValues[j]));
          }
          arr = arr.add(P_LENGTH, valueProperty(l.abst1(i)));
  ...
  MemberKont.prototype.apply =
      function (objectValue, store, lkont, kont)
      {
        ...
        let objectRef;
        ({value: objectRef, store} = base.ToObject(objectValue, store, lkont, kont, benv.addresses()));
        if (node.computed)
        {
        ...
        const propValue = lookupProperty(objectRef, l.abst1(property.name), store);
        const resultStates = [{state: new KontState(propValue, store, lkont, kont), effects: []}];
        return resultStates;
      }
  ...
  MemberPropertyKont.prototype.apply =
      function (propertyValue, store, lkont, kont)
      {
        ...
        const value = lookupProperty(objectRef, nameValue, store);
        return [{state: new KontState(value, store, lkont, kont), effects: []}];
      }
  ...
  CallMemberKont.prototype.apply =
      function (objectValue, store, lkont, kont, realm, stacks, contexts, sstorei)
      {
        var node = this.node;
        var benv = this.benv;
        ...
        var operands = node.arguments;
        return invoke(node, thisAddresses, nameValue, operands, benv, store, lkont, kont, realm, stacks, contexts, sstorei);
      }
  
  function invoke(application, thisAddresses, nameValue, operands, benv, store, lkont, kont, realm, stacks, contexts, sstorei)
  {
    return thisAddresses.flatMap(
        function (thisa)
        {
          var effects = [];
          var operatorValue = lookupProperty(l.abstRef(thisa)/*TODO*/, nameValue, store);
          if (operands.length === 0)
          {
            if (Ast.isNewExpression(application))
            {
              return applyCons(application, operatorValue, [], benv, store, lkont, kont, effects, stacks, contexts, sstorei, realm);
            }
            return applyProc(application, operatorValue, [], thisa, null, store, lkont, kont, effects, stacks, contexts, sstorei, realm);
          }
  ...
  UpdateMemberKont.prototype.apply =
      function (objectRef, store, lkont, kont)
      {
        ...
        var effects = [];
        var value = lookupProperty(objectRef, name, store, effects);
        if (value === BOT)
        {
          return [];
        }
        ...
        store = setProperty(objectRef, name, updatedValue, store, effects);
  ...
  MemberAssignmentValueKont.prototype.apply =
      function (value, store, lkont, kont)
      {
        ...
        store = setProperty(objectRef, nameValue, newValue, store, effects);
        return [{state: new KontState(newValue, store, lkont, kont), effects: effects}];
      }
  
  
  function doScopeLookup(nameNode, benv, store, effects, realm)
  {
    const globala = realm.GlobalObject;
  ...
  function doScopeSet(nameNode, value, benv, store, effects, realm)
  {
    const globala = realm.GlobalObject;
    var name = nameNode.name;
    var a = benv.lookup(name);
    if (a === BOT)
    {
      var obj = storeLookup(store, globala);
      var aname = l.abst1(name);
      obj = obj.add(aname, valueProperty(value));
  ...
  function evalIdentifier(node, benv, store, lkont, kont, effects, realm)
  {
    var value = doScopeLookup(node, benv, store, effects, realm);
  ...
  function evalProgram(node, benv, store, lkont, kont, effects, realm)
  {
    assert(realm);
    assert(realm.Intrinsics);
    const globala = realm.GlobalObject;
    ...
    if (names.length > 0)
    {
      ...
      names.forEach(
          function (name)
          {
            ...
            {
              var allocateResult = allocateClosure(node, benv, store, realm);
              var closureRef = allocateResult.ref;
              store = allocateResult.store;
              obj = obj.add(aname, valueProperty(closureRef));
              effects.push(writeObjectEffect(globala, aname));
            }
            else if (Ast.isVariableDeclarator(node))
            {
              obj = obj.add(aname, valueProperty(L_UNDEFINED));
              effects.push(writeObjectEffect(globala, aname));
            }
            else
            {
    ...
    return evalStatementList(node, benv, store, lkont, kont, effects, realm);
  }
  
  function evalStatementList(node, benv, store, lkont, kont, effects, realm)
  {
    assert(realm);
    assert(realm.Intrinsics);
    var nodes = node.body;
...
//      return KONT.unch(new EvalState(nodes[0], benv, store));
      return evalNode(nodes[0], benv, store, lkont, kont, effects, realm);
    }
  ...
  function evalUpdateExpression(node, benv, store, lkont, kont, effects, realm)
  {
    var argument = node.argument;
    switch (argument.type)
    {
      ...
      {
        var effects = [];
        var value = doScopeLookup(argument, benv, store, effects, realm);
        var updatedValue;
        switch (node.operator)
        {
          case "++":
        ...
        store = doScopeSet(argument, updatedValue, benv, store, effects, realm);
        var resultingValue = node.prefix ? updatedValue : value;
        return [{state: new KontState(resultingValue, store, lkont, kont), effects: effects}];
      }
      case "MemberExpression":
  ...
  function evalFunctionExpression(node, benv, store, lkont, kont, effets, realm)
  {
    assert(realm);
    assert(realm.Intrinsics);
    const strict = true; // TODO
    let closure;
    ({closure, store} = FunctionCreate("Normal", node, benv, strict, store, realm));
    store = MakeConstructor(closure, store);
    return [{state: new KontState(closure, store, lkont, kont)}];
  }
  ...
  function evalObjectExpression(node, benv, store, lkont, kont, effects, realm)
  {
    var properties = node.properties;
...
//      effects.push(allocObjectEffect(objectAddress));
  ...
  function evalArrayExpression(node, benv, store, lkont, kont, effects, realm)
  {
    var elements = node.elements;
    if (elements.length === 0)
    {
      var arr = createArray(realm);
      arr = arr.add(P_LENGTH, valueProperty(L_0));
      var arrAddress = a.array(node, benv, store, lkont, kont);
      store = storeAlloc(store, arrAddress, arr);
  ...
  function evalNode(node, benv, store, lkont, kont, effects, realm)
  {
    assert(realm);
    assert(realm.Intrinsics);
    assert(Array.isArray(effects));
    //print(NODE.toString());
    return evalNonAtomic(node, benv, store, lkont, kont, effects, realm);
  }
  
  function evalNonAtomic(node, benv, store, lkont, kont, effects, realm)
  {
    switch (node.type)
    {
      case "Literal":
        return evalLiteral(node, benv, store, lkont, kont, effects, realm);
      case "Identifier":
        return evalIdentifier(node, benv, store, lkont, kont, effects, realm);
      case "BinaryExpression":
        return evalBinaryExpression(node, benv, store, lkont, kont, effects, realm);
      case "LogicalExpression":
        return evalLogicalExpression(node, benv, store, lkont, kont, effects, realm);
      case "CallExpression":
        return evalCallExpression(node, benv, store, lkont, kont, effects, realm);
      case "FunctionExpression":
        return evalFunctionExpression(node, benv, store, lkont, kont, effects, realm);
      case "AssignmentExpression":
        return evalAssignmentExpression(node, benv, store, lkont, kont, effects, realm);
      case "ArrayExpression":
        return evalArrayExpression(node, benv, store, lkont, kont, effects, realm);
      case "MemberExpression":
        return evalMemberExpression(node, benv, store, lkont, kont, effects, realm);
      case "ObjectExpression":
        return evalObjectExpression(node, benv, store, lkont, kont, effects, realm);
      case "ThisExpression":
        return evalThisExpression(node, benv, store, lkont, kont, effects, realm);
      case "NewExpression":
        return evalCallExpression(node, benv, store, lkont, kont, effects, realm);
      case "UpdateExpression":
        return evalUpdateExpression(node, benv, store, lkont, kont, effects, realm);
      case "UnaryExpression":
        return evalUnaryExpression(node, benv, store, lkont, kont, effects, realm);
      case "ExpressionStatement":
        return evalNode(node.expression, benv, store, lkont, kont, effects, realm);
      case "ReturnStatement":
        return evalReturnStatement(node, benv, store, lkont, kont, effects, realm);
      case "BreakStatement":
        return evalBreakStatement(node, benv, store, lkont, kont, effects, realm);
      case "LabeledStatement":
        return evalLabeledStatement(node, benv, store, lkont, kont, effects, realm);
      case "IfStatement":
        return evalIfStatement(node, benv, store, lkont, kont, effects, realm);
      case "ConditionalExpression":
        return evalConditionalExpression(node, benv, store, lkont, kont, effects, realm);
      case "SwitchStatement":
        return evalSwitchStatement(node, benv, store, lkont, kont, effects, realm);
      case "ForStatement":
        return evalForStatement(node, benv, store, lkont, kont, effects, realm);
      case "WhileStatement":
        return evalWhileStatement(node, benv, store, lkont, kont, effects, realm);
      case "DoWhileStatement":
        return evalDoWhileStatement(node, benv, store, lkont, kont, effects, realm);
      case "FunctionDeclaration":
        return evalFunctionDeclaration(node, benv, store, lkont, kont, effects, realm);
      case "VariableDeclaration":
        return evalVariableDeclaration(node, benv, store, lkont, kont, effects, realm);
      case "VariableDeclarator":
        return evalVariableDeclarator(node, benv, store, lkont, kont, effects, realm);
      case "BlockStatement":
        return evalStatementList(node, benv, store, lkont, kont, effects, realm);
      case "EmptyStatement":
        return evalEmptyStatement(node, benv, store, lkont, kont, effects, realm);
      case "TryStatement":
        return evalTryStatement(node, benv, store, lkont, kont, effects, realm);
      case "ThrowStatement":
        return evalThrowStatement(node, benv, store, lkont, kont, effects, realm);
      case "Program":
        return evalProgram(node, benv, store, lkont, kont, effects, realm);
      default:
  ...
  function createError(message, realm)
  {
    ...
    let obj = new Obj(realm.Intrinsics.get("%ErrorPrototype%"));
    obj = obj.add(l.abst1("[[ErrorData]]"), L_UNDEFINED);
    obj = obj.add(P_MESSAGE, valueProperty(message));
    return obj;
  }
  ...
  function initialize(store)
  {
    ...
    const realm0 = new Realm();
    const intrinsics = new Intrinsics();
    realm0.Intrinsics = intrinsics;
    
    const globala = allocNative();
    realm0.GlobalObject = globala; // TODO mmm
    
    
    const realm = Object.freeze(realm0);
    ...
    {
      object = object.add(l.abst1(propertyName), valueProperty(value));
      return object;
    }
...
//  objectP.toString = function () { return "~Object.prototype"; }; // debug
    ...
    object = object.add(P_PROTOTYPE, valueProperty(objectProtoRef));//was objectProtoRef
    global = global.add(l.abst1("Object"), valueProperty(l.abstRef(objecta)));
...
//  functionP.toString = function () { return "~Function.prototype"; }; // debug
    ...
    fun = fun.add(P_PROTOTYPE, valueProperty(functionProtoRef));
    global = global.add(l.abst1("Function"), valueProperty(l.abstRef(functiona)));
    ...
    var errorP = createObject(intrinsics.get("%ObjectPrototype%"));
    ...
    error = error.add(P_PROTOTYPE, valueProperty(errorProtoRef));
    global = global.add(l.abst1("Error"), valueProperty(l.abstRef(errora)));
    store = storeAlloc(store, errora, error);
    store = storeAlloc(store, errorPa, errorP);
    
    function errorFunction(application, operandValues, thisa, benv, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
    {
      return errorInitializer(application, operandValues, benv, store, lkont, kont, effects, realm);
    }
    
    function errorConstructor(application, operandValues, protoRef, benv, store, lkont, kont, effects, stacks, contexts, sstorei, realm)
    {
      return errorInitializer(application, operandValues, benv, store, lkont, kont, effects, realm);
    }
    
    
    function errorInitializer(application, operandValues, benv, store, lkont, kont, effects, realm)
    {
      const message = operandValues.length === 1 && operandValues[0] !== BOT ? operandValues[0].ToString() : L_EMPTY_STRING;
      const obj = createError(message, realm);
    ...
    var stringP = createObject(intrinsics.get("%ObjectPrototype%"));
    ...
    string = string.add(P_PROTOTYPE, valueProperty(intrinsics.get("%StringPrototype%")));
    global = global.add(l.abst1("String"), valueProperty(l.abstRef(stringa)));
    ...
    function stringCharAt(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      var lprim = str.lookup(l.abst1("[[StringData]]"));
    ...
    var arrayP = createObject(intrinsics.get("%ObjectPrototype%"));
    ...
    array = array.add(P_PROTOTYPE, valueProperty(arrayProtoRef));
    global = global.add(l.abst1("Array"), valueProperty(l.abstRef(arraya)));
    ...
    arrayP = registerPrimitiveFunction(arrayP, "pop", arrayPop);
...
//  arrayP = registerPrimitiveFunction(arrayP, arrayPa, "reduce", arrayReduce);
    ...
    function arrayConstructor(application, operandValues, protoRef, benv, store, lkont, kont, effects)
    {
      ...
      arr = arr.add(P_LENGTH, valueProperty(length));
    ...
    function arrayFunction(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      {
        arr = arr.add(l.abst1(String(i)), valueProperty(operandValues[i]));
      }
      arr = arr.add(P_LENGTH, valueProperty(l.abst1(operandValues.length)));
    ...
    function arrayToString(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      while ((!seen.contains(i)) && l.lt(i, len).isTrue())
      {
        ...
        var v = lookupProperty(l.abstRef(thisa) /*TODO*/, iname, store, effects);
        if (v !== BOT)
        {
          r.push(v);
        }
    ...
    function arrayConcat(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      var resultArr = createArray(realm);
      var i = L_0;
      var seen = ArraySet.empty();
      while ((!seen.contains(i)) && l.lt(i, thisLen).isTrue())
      {
        seen = seen.add(i);
        var iname = i.ToString();
        var v = lookupProperty(l.abstRef(thisa), iname, store, effects);
        resultArr = resultArr.add(iname, valueProperty(v));
        i = l.add(i, L_1);
      }
      argAddrs.forEach(
          function (argAddr)
          {
            ...
            while ((!seen.contains(i)) && l.lt(i, argLen).isTrue())
            {
              seen = seen.add(i);
              var iname = i.ToString();
              var v = lookupProperty(l.abstRef(argAddr), iname, store, effects);
              resultArr = resultArr.add(l.add(thisLen, i).ToString(), valueProperty(argArr.lookup(iname).value));
              i = l.add(i, L_1);
            }
            resultArr = resultArr.add(P_LENGTH, valueProperty(l.add(thisLen, i)));
          });
    ...
    function arrayPush(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      arr = arr.add(lenStr, valueProperty(operandValues[0]))
      effects.push(writeObjectEffect(thisa, lenStr));
      var len1 = l.add(len, L_1);
      arr = arr.add(P_LENGTH, valueProperty(len1));
    ...
    function arrayPop(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      var arr = storeLookup(store, thisa);
      var len = arr.lookup(P_LENGTH).value;
      var len1 = l.sub(len, L_1);
      var lenStr = len1.ToString();
      var d = arr.lookup(lenStr).value;
      arr = arr.add(P_LENGTH, valueProperty(len1));
      store = storeUpdate(store, thisa, arr);
      return [{state: new KontState(d, store, lkont, kont), effects: effects}];
    }
    ...
    // BEGIN MATH
    var math = createObject(intrinsics.get("%ObjectPrototype%"));
...
//  math = registerProperty(math, "PI", l.abst1(Math.PI));
    ...
    global = global.add(l.abst1("Math"), valueProperty(l.abstRef(matha)));
    ...
    // BEGIN BASE
    var base = createObject(l.abst1(null));
    ...
    base = registerPrimitiveFunction(base, "get", baseGet);
    store = storeAlloc(store, basea, base);
    global = global.add(l.abst1("$BASE$"), valueProperty(l.abstRef(basea)));
    ...
    function baseAddMeta(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
      ...
      global = global.add(Name, Value);
      store = storeUpdate(store, globala, global);
      return [{state: new KontState(l.abst1(undefined), store, lkont, kont), effects: effects}];
    }
  
    function baseGet(application, operandValues, thisa, benv, store, lkont, kont, effects)
    {
    ...
    // BEGIN PERFORMANCE
    let perf = createObject(realm.Intrinsics.get("%ObjectPrototype%"));
  ...
  function explore(ast, ceskState)
  {
    ...
    const realm = ceskState.realm;
    
    assert(store);
    assert(realm);
    
    const kont = createContext(ast, realm.GlobalObject, "globalctx", ArraySet.from1(realm.GlobalObject), null, contexts, sstorei);
    const initialState = new EvalState(ast, Benv.empty(), store, [], kont);
    
    const system = performExplore([initialState], realm, stacks, contexts, sstorei);
    return system;
  }
  
  function performExplore(initialStates, realm, stacks, contexts, sstorei)
  {
    assert(realm);
    assert(realm.Intrinsics);
    assert(stacks);
    ...
    while (todo.length > 0)
    {
      ...
      var s = todo.pop();
      if (s._sstorei === sstorei[0])
      {
        continue;
      }
      s._sstorei = sstorei[0];
      ...
      next = s.next(realm, stacks, contexts, sstorei);
      ...
      for (var i = 0; i < next.length; i++)
      {
        ...
        {
          print(Formatter.displayTime(performance.now() - startTime), "states", states.length, "todo", todo.length, "ctxs", contexts.length, "sstorei", sstorei[0]);
        }
      }
    }
  ...
  function runMain(operands, ceskState)
  {
    const sstorei = [0];
    const stacks = []; // do not pre-alloc
    const contexts = []; // do not pre-alloc
    ...
    const globala = realm.GlobalObject;
    const global = storeLookup(store, globala);
    const operator = global.lookup(l.abst1("main")).value;
    if (!operator)
    {
      throw new Error("function 'main' not found");
    }
    let value;
    ({store, value} = allocIntoStore(operands, store, realm));
    const kont2 = createContext(null, globala, "main", value.addresses(), null, contexts, sstorei);
    const states = applyProc(null, operator, [value], globala, null, store, [], kont2, [], stacks, contexts, sstorei, realm).map((t) => t.state);
    const system = performExplore(states, realm, stacks, contexts, sstorei);
    return system;
  }
  
  
  function allocIntoStore(v, store, realm)
  {
    function walker(v)
    {
      if (v === null)
      ...
      if (Array.isArray(v))
      {
        let arr = createArray(realm);
        for (const key in v)
        {
          arr = arr.add(l.abst1(key), valueProperty(walker(v[key])))
        }
        arr = arr.add(P_LENGTH, valueProperty(l.abst1(v.length)));
        const addr = a.object();
        store = storeAlloc(store, addr, arr);
        return l.abstRef(addr);
      }
      if (typeof v === "object")
      {
        let obj = createObject(L_NULL);
        for (const key in v)
        {
          obj = obj.add(l.abst1(key), valueProperty(walker(v[key])))
        }
        const addr = a.object();
...
function retrieveEndStates(initial) // not used for the moment
{
  ...
  while (todo.length > 0)
  {
    ...
    {
      todo = todo.concat(successors.map(function (t)
      {
        return t.state
      }));
    }
