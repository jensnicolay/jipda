...
export function computeInitialCeskState(lat, alloc, kalloc, ...srcs)
{
  ...
  const queues = [...prelResult][0].queues();

  const ceskState = {store, realm, queues};
  return ceskState;
}
...
export function jsCesk(lat, alloc, kalloc, cc)
{
  ...
  const queues = new Map(); // name -> address
  ...
  function inject(initialState)
  {
    ...
    const operations =
        {
          evaluate: function (node, benv, store, lkont, kont)
          {
            return new EvalState(node, benv, store, lkont, kont)
          }
        }
    ...
    {
      const queueA = queues.get(queueName);
...
//      const arrAddress = alloc.array(node, kont);
        ...
        function ()
        {
          ...
          if (lkont.length === 0)
          {
            if (kont._stacks.size === 0)
            {
              ...
              const result = jobStores.map(jobStore => jobStore[0].execute(operations, jobStore[1], lkont, kont)).flatten();
              return result;
            }
    ...
    KontState.prototype.enqJobs =
        function (queueName, jobs)
        {
          let store = this.store;
          for (const job of jobs)
          {
            store = enqueueJob(queueName, job, store);
          }
    ...
    BreakState.prototype.currentStackStore =
        function ()
        {
          return sstorei;
        }
...
//      return kont.unch(new EvalState(nodes[0], benv, store));
    ...
    function initialize(benv, store)
    {
      ...
      queues.set("ScriptJobs", "ScriptJobs");
      store = storeAlloc(store, "ScriptJobs", L_NULL);
...
//  math = registerProperty(math, "PI", lat.abst1(Math.PI));
      ...
      return {store, realm, queues};
    } // end initialize
  
  
    initialState = initialState || initialize(Benv.empty(), Store.empty());
    const store = initialState.store;
    const realm = initialState.realm;
    for (const [key, value] of initialState.queues)
    {
      queues.set(key, value);
    }
    const kont = createContext(null, realm.GlobalObject, realm, "globalctx", ArraySet.empty().add("ScriptJobs"), null);
    const koState = new KontState(L_UNDEFINED, store, [], kont);
    return koState;
  }
  
  return inject(cc.initialState);
}
...
export function performExplore(initialStates)
{
  ...
  while (todo.length > 0)
  {
    ...
    var s = todo.pop();
    const sstorei = s.currentStackStore();
...
ScriptEvaluationJob.prototype.execute =
    function (operations, store, lkont, kont)
    {
      const ast = Ast.createAst(this.src);
      return [operations.evaluate(ast, kont.realm.GlobalEnv, store, lkont, kont)];
    }
...
ScriptEvaluationJob.prototype.hashCode =
        function ()
        {
          ...
          result = prime * result + HashCode.hashCode(this.src);
          return result;
        }
